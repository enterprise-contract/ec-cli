...
# utilities: kubectl & openssl; and access to an operating HACBS cluster.
...
# The default in HACBS
PIPELINE_SERVICE_ACCOUNT=pipeline

# What pipeline bundle to use
PIPELINE_BUNDLE=quay.io/redhat-appstudio/hacbs-core-service-templates-bundle:devel

# Where to push the image(s)
IMAGE_REPOSITORY=quay.io/hacbs-contract-demo

# If HACBS is not setup, there might not be a appstudio PVC, this creates if if
...
# could be empty in HACBS, so it needs to be recreated from the local Docker
# configuration
{
...
kubectl get secret -n tekton-chains signing-secrets -o jsonpath='{.data.cosign\.pub}'|base64 -d > "${HACK_DIR}/cosign.pub"

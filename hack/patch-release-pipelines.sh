...
for p in pull-request push; do
  ...
  changes=$( git log main --reverse --pretty=%h --invert-grep \
    --grep="Update RHTAP references" --grep="Red Hat Trusted App Pipeline update" -- $main_pipeline )
...
# Make the commit
git commit -m "chore: Pipeline mods for release $release_name" \
  -m "Commit created with hack/patch-release-pipelines.sh"

# Invite human to look at it
